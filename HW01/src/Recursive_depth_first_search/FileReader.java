package Recursive_depth_first_search;
...
public class FileReader {
	...
	try{
		...
		RecursiveDFS.setMax_Row(row-1);
		col=Integer.parseInt(temp[1]);
		RecursiveDFS.setMax_Col(col-1);
		line=d.readLine();
		line=line.trim();
		temp=line.split(" ");
		int startx=Integer.parseInt(temp[0]);
		RecursiveDFS.setStartx(startx);
		int starty=Integer.parseInt(temp[1]);
		RecursiveDFS.setStarty(starty);
		int j=i=0;
		while(i<row)
		{
			line=d.readLine();
		...
		RecursiveDFS.setGraph(Graph);
		RecursiveDFS.setGoals(Message_Obtained);
		getpos();
		i=0;
		while(i<row)
		{
		...
		RecursiveDFS.setMap_cost(cost_map);
	}catch(IOException e)
	{
	...
	private static void getpos() {
		...
		RecursiveDFS.setGoal_pos(Goal_pos);
	}
}
